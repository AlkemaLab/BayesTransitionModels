...
#' Fit the transitionmodel model to data.
#'
...
#'
#' @return transitionmodel object.
#'
#' @details
#' Several area-specific parameters of the transitionmodel model have hierarchical priors
...
transitionmodel <- function(
  ...
  ###### Load model #####
  include_paths <- system.file("include", package = "transitionmodel")
  stan_file_path <- system.file("stan/fpem_spline.stan", package = "transitionmodel")
  if(model == "spline") {
    ...
    stan_code <- readr::read_file(model) %>%
      str_replace_all("\\{\\{RHO_PRIOR\\}\\}", rho_prior) %>%
      str_replace_all("\\{\\{TAU_PRIOR\\}\\}", tau_prior)
  ...
  result$posteriors <- process_fit(result, ifelse(is.null(args$parallel_chains), 1, args$parallel_chains))

  attr(result, "class") <- "transitionmodel"

  result
}

#' Print transitionmodel object
#'
#' @param x transitionmodel object
#' @param ... additional arguments (not currently used)
#'
#' @importFrom glue glue
#' @export
print.transitionmodel <- function(x, ...) {
  cat("Fitted transitionmodel model for ")
  cat(glue::glue("{x$stan_data$C} {pluralize(x$stan_data$C, 'area', 'areas')} and {x$stan_data$T} {pluralize(x$stan_data$T, 'time point', 'time points')}"))

  cat("\nDiagnostics:\n")
  diagnostics <- x$samples$diagnostic_summary()
}
